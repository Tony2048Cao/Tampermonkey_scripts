...
(function() {
    ...
    const siteConfigs = {
        zhihu: {
            containerSelector: '.ContentItem, .TopstoryItem--advertCard',
    ...
    function processContentElement(element, config) {
        ...
        // 新增：屏蔽知乎广告
        if (site === 'zhihu' && element.matches && element.matches('.TopstoryItem--advertCard')) {
            element.remove();
            console.log(`${config.logPrefix}: 屏蔽广告`);
            return;
        }

        // 新增功能：如果知乎评论区被筛选，则直接屏蔽
        if (site === 'zhihu') {
            // 感谢你提供的HTML信息！下面将使用更可靠的文本内容来检测
            const divs = element.querySelectorAll('div');
            for (const div of divs) {
                if (div.textContent.trim() === '评论内容由作者筛选后展示') {
                    let cardElement = element.closest('.Card.TopstoryItem.TopstoryItem-isRecommend');
                    if (cardElement) {
                        cardElement.remove();
                        console.log(`${config.logPrefix}: 因评论区被筛选而屏蔽 (删除整个卡片)`);
                        return; // 屏蔽后，无需再检查关键词
                    }
                    // 如果找不到外层卡片，也直接跳出循环
                    break;
